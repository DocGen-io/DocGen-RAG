...
class TestFallbackDocumentation:
    """Verify fallback documentation remains OpenAPI compliant."""

    def test_fallback_has_no_custom_security_keys(self):
        """Fallback docs should NOT introduce non-standard keys."""
        with patch.object(DocumentationCreator, '__init__', lambda self: None):
            creator = DocumentationCreator()
            method = {
            ...
            fallback = creator._create_fallback_documentation(method)

            swagger = fallback["swagger"]
            allowed_keys = {
                "summary", "description", "operationId", "parameters",
                "requestBody", "responses", "security", "tags",
                "deprecated", "callbacks", "servers", "externalDocs"
            ...
            for key in swagger:
                assert key in allowed_keys, \
                    f"Fallback has non-standard key: '{key}'"

    def test_fallback_description_is_valid_string(self):
        """Fallback description should be a non-empty string."""
        with patch.object(DocumentationCreator, '__init__', lambda self: None):
            creator = DocumentationCreator()
            method = {
                "method_name": "testMethod",
            ...
            fallback = creator._create_fallback_documentation(method)
            assert isinstance(fallback["swagger"]["description"], str)
            assert len(fallback["swagger"]["description"]) > 0
